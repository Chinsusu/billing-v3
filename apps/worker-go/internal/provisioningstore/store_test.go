...
func TestMarkProcessedActivatesServiceAndMarksJobProcessed(t *testing.T) {
	...
	mock.ExpectExec(regexp.QuoteMeta(`update services set status = 'active', external_id = $1, config = $2, provisioned_at = coalesce($3, now()), expires_at = coalesce($4, expires_at), updated_at = now() where id = $5`)).
		...
		WillReturnResult(sqlmock.NewResult(0, 1))
	mock.ExpectExec(regexp.QuoteMeta(`insert into notification_events (id, user_id, channel, type, recipient_email, subject, body_text, source_type, source_id, idempotency_key, status, attempts, max_attempts, available_at, payload, created_at, updated_at) select $1, services.user_id, 'email', 'service_provisioned', users.email, $2, $3, 'service', services.id::text, $4, 'pending', 0, 3, now(), $5, now(), now() from services join users on users.id = services.user_id where services.id = $6 on conflict (idempotency_key) do nothing`)).
