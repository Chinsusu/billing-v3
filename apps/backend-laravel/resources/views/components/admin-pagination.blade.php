...
@if ($paginator->hasPages())
    <nav
        {{ $attributes->merge(['class' => 'admin-pagination']) }}
        data-admin-pagination="{{ $name }}"
    ...
    >
        ...
        <div class="admin-pagination__controls">
            @if ($paginator->onFirstPage())
                <span class="admin-pagination__control is-disabled" aria-disabled="true" aria-label="Previous page">
                    <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M15.41 7.41 14 6l-6 6 6 6 1.41-1.41L10.83 12l4.58-4.59Z"/></svg>
                </span>
            @else
                <a class="admin-pagination__control" href="{{ $paginator->previousPageUrl() }}" rel="prev" aria-label="Previous page">
                    <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M15.41 7.41 14 6l-6 6 6 6 1.41-1.41L10.83 12l4.58-4.59Z"/></svg>
                </a>
            @endif

            @if ($windowStart > 1)
                <a class="admin-pagination__page" href="{{ $paginator->url(1) }}">1</a>
                @if ($windowStart > 2)
            ...
            @if ($windowEnd < $lastPage)
                @if ($windowEnd < $lastPage - 1)
                    <span class="admin-pagination__ellipsis" aria-hidden="true">...</span>
                @endif
                <a class="admin-pagination__page" href="{{ $paginator->url($lastPage) }}">{{ $lastPage }}</a>
            @endif

            @if ($paginator->hasMorePages())
                <a class="admin-pagination__control" href="{{ $paginator->nextPageUrl() }}" rel="next" aria-label="Next page">
                    <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M8.59 16.59 13.17 12 8.59 7.41 10 6l6 6-6 6-1.41-1.41Z"/></svg>
                </a>
            @else
                <span class="admin-pagination__control is-disabled" aria-disabled="true" aria-label="Next page">
                    <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M8.59 16.59 13.17 12 8.59 7.41 10 6l6 6-6 6-1.41-1.41Z"/></svg>
                </span>
            @endif
        </div>
